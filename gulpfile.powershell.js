/**
 * FeHelper Chrome Extension Builder By Gulp (PowerShell Compatible)
 * @author zhaoxianlie
...
function zipPackage(outputRoot = 'output-chrome', cb) {
    ...
    // PowerShell-compatible zip creation using Compress-Archive
    const zipPath = path.join(outputRoot, pkgName);
    const appsPath = path.join(outputRoot, 'apps');
    
    // Remove existing zip file if it exists
    if (fs.existsSync(zipPath)) {
        shell.exec(`powershell -Command "Remove-Item -Path '${zipPath}' -Force"`);
    }
    
    // Create zip using PowerShell Compress-Archive
    const psCommand = `powershell -Command "Compress-Archive -Path '${appsPath}\\*' -DestinationPath '${zipPath}' -Force"`;
...
function firefoxPreprocess(cb) {
    ...
    shell.exec(`powershell -Command "Move-Item -Path '${destDir}\\firefox.json' -Destination '${destDir}\\manifest.json' -Force"`);
    shell.exec(`powershell -Command "Remove-Item -Path '${destDir}\\chrome.json' -Force -ErrorAction SilentlyContinue"`);
    shell.exec(`powershell -Command "Remove-Item -Path '${destDir}\\edge.json' -Force -ErrorAction SilentlyContinue"`);

    FIREFOX_REMOVE_TOOLS.forEach(tool => {
        const toolDir = path.join(destDir, tool);
        if (fs.existsSync(toolDir)) {
            shell.exec(`powershell -Command "Remove-Item -Path '${toolDir}' -Recurse -Force -ErrorAction SilentlyContinue"`);
        }
    });
    cb();
}
...
function chromePreprocess(cb) {
    ...
    shell.exec(`powershell -Command "Move-Item -Path '${destDir}\\chrome.json' -Destination '${destDir}\\manifest.json' -Force"`);
    shell.exec(`powershell -Command "Remove-Item -Path '${destDir}\\firefox.json' -Force -ErrorAction SilentlyContinue"`);
    shell.exec(`powershell -Command "Remove-Item -Path '${destDir}\\edge.json' -Force -ErrorAction SilentlyContinue"`);
...
function edgePreprocess(cb) {
    ...
    shell.exec(`powershell -Command "Move-Item -Path '${destDir}\\edge.json' -Destination '${destDir}\\manifest.json' -Force"`);
    shell.exec(`powershell -Command "Remove-Item -Path '${destDir}\\chrome.json' -Force -ErrorAction SilentlyContinue"`);
    shell.exec(`powershell -Command "Remove-Item -Path '${destDir}\\firefox.json' -Force -ErrorAction SilentlyContinue"`);
